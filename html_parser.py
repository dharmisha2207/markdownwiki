...
class HTMLScrolledText(_ScrolledText):
    ...
    def __init__(self, *args, html=None, **kwargs):
        ...
        self._w_init(kwargs)
        self.html_parser = HTMLTextParser(self.frame)
...
class HLinkSlot():
    ...
    def __init__(self,f, w, tag_name, url):
        #------------------------------------------------------------------------------------------
        self._w = w
        self.frame=f
    ...
    def call(self, event):
        ...
        for w in self.frame.winfo_children():
            w.destroy()
        f=open("Html_Articles/"+self.URL)
        txt = f.read()

        htmllabel=HTMLLabel(self.frame,background="white",height=450)
        htmllabel.set_html(txt)
        globalvars.selfile=self.URL[:-5]
    ...
    def _w_tags_apply_all(self):
        ...
        for key, tag in self._w_tags.items():
            ...
            if tag[Bind.KEY][Bind.LINK]:
                self.hlink_slots.append(HLinkSlot(self.frame,self._w, key, tag[Bind.KEY][Bind.LINK]))
